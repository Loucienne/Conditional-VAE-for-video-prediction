...
class Dataset_Dance(torchData):
    ...
    def __init__(self, root, transform, mode='train', video_len=7, partial=1.0):
        ...
        if mode == 'train':
            #print(sorted(glob(os.path.join(root, 'train/train_img/*.png')), key=get_key))
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.video_len):
            ...
            '''
            #Split by \\
            label_list_inter = self.img_folder[(index*self.video_len)+i].split('\\')
            # Then split each element by '/'
            label_list = []
            for part in label_list_inter:
