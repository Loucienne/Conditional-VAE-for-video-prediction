...
def get_key(fp):
    ...
    return int(filename)
"""
def get_key(fp):    #to remove (maybe ?)
    filename = os.path.basename(fp)
    filename = filename.split('.')[0].replace('frame', '')
    try:
        return int(filename)
    except ValueError:
        print(f"Error converting {filename} to an integer. Returning 0 as a fallback.")
...
class Test_model(VAE_Model):
    ...
    def forward(self, img, label):
        ...
        # Run trought the decoder fusion
        decoded = self.Decoder_Fusion(img_encoded, label_encoded, z)
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in tqdm(range(1,label.shape[0])):
            ...
            generated_image = self.forward(decoded_frame_list[i-1], label[i])
            # complete the frame and label lists
            decoded_frame_list.append(generated_image)
    ...
    def load_checkpoint(self):
        if self.args.ckpt_path != None:
            checkpoint = torch.load(self.args.ckpt_path, map_location=torch.device(self.args.device))
